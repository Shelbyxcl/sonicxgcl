...
function App() {
  ...
  const fetchDynamicData = async () => {
    try {
      ...
      const balance = await brettMinerContract.getBalance(); // Balance du contrat (TVL en ETH)
      const tokenPrice = await fetchTokenPrice(); // Prix du token en USD
      const miners = await brettMinerContract.getMyMiners(); // Récupération des miners
      const rewardInBRETT = userReward;
      ...
      return {
        tvlETH,
        tvlUSD,
        balance: brettBalance,
        reward: rewardInBRETT,
        tokenPrice: brettPrice,
        claimPower,
        ...
        >
          ...
          <div style={{ paddingTop: "15px", paddingBottom: "15px" }}>
            <p className="custom-font" style={{ textAlign: "left" }}>
              Brett Miner's referral program allows each user to get a{" "}
